...
class SlackService:
    ...
    async def list_channels(self) -> List[SlackChannel]:
        ...
        sample_thread = SlackThread(
            id="T01",
            rootMessageId="M01",
            lastActivityTs=str(now),
            messages=[
                SlackMessage(id="M01", userId="U01", text="Welcome to Employee Pulse!", ts=str(now - 86400), sentiment=0.6),
                SlackMessage(id="M02", userId="U02", text="Let's keep an eye on burnout and PTO coverage.", ts=str(now - 86000), sentiment=0.1),
            ],
        )
        return [
